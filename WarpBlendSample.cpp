...
int main(int argc, char **argv)
{
	...
	for (gpu = 0; gpu < gpuCount; gpu++) 
	{
		...
		for (int dispIndex = 0; (dispIndex < dispIdCount &&dispIndex < 1) && dispIds[dispIndex].isActive; dispIndex++)
		{
			...
			if (error == NVAPI_OK) 
			{
				...
				//scanoutInformation.targetViewportRect = { 960,0,1920,1080 };
...
*/
			...
			//     x                        y                  u                           v                   r       q
			{
				...
				dstLeft,				     dstTop,				srcLeft + dstXShift,					srcTop,				0.0f,	 1.0f,     // 0
				dstLeft ,					dstTop + dstHeight,		srcLeft + dstXShift,					srcTop + srcHeight,	0.0f,	 1.0f,     // 1
				dstLeft + dstWidth,			dstTop,					srcLeft + dstXShift+srcWidth,			srcTop,				0.0f,	 1.0f,      // 2
				dstLeft + dstWidth,	         dstTop + dstHeight,	srcLeft + dstXShift +srcWidth,			srcTop + srcHeight,	0.0f,	 1.0f,     // 3
			};
			...
			float intensityTexture[12] = { 0.95f, 0, 0, 0.75f, 0, 0, 0.25f, 0, 0, 0.05f, 0, 0 };
